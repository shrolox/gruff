...
class Gruff::BarNLine < Gruff::Bar
  ...
  attr_accessor :show_labels_for_lines_values

  attr_accessor :line_label_formatting
  ...
  def draw_bars
    ...
    col_space = @label_bar_group_spacing * @graph_width
    ...
    @norm_data.each_with_index do |data_row, row_index|
      ...
      data_row[DATA_VALUES_INDEX].each_with_index do |data_point, index|
        ...
        if @show_labels_for_lines_values
          val = (@line_label_formatting || '%.2f') % @norm_data[row_index][3][index]
          draw_value_label(new_x, new_y-40, val, true)
        end
